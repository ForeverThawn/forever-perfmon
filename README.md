...
The app reads `forever-permon.toml` from the repository root:
...
It writes a timestamped CSV file and `snapshot.json` into the configured directories. If a snapshot already exists, press `C` to continue cumulative counters or `R` to reset them. No Enter key is required.
